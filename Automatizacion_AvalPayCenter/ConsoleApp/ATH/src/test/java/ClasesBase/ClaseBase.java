...
// METODOS BASE -->	
	...
	public void visibilityOfElementLocated(By locator, long tiempo) {
	...
	public void cargarArchivo(String archivo, By locator, File rutaCarpeta, String generarEvidencia, int tiempo,
		...
		sendkey(path, locator, rutaCarpeta, generarEvidencia, steps);
	...
	public void click(By locator, File rutaCarpeta, String generarEvidencia, String steps) throws Exception {
		if (generarEvidencia.equals("SI")) {
			try {
				...
				// ScreenShotError(rutaCarpeta, locator, generarEvidencia, steps, "NO FUE
				// POSIBLE LOCALIZAR EL ELEMENTO: (" +locator+ ") PARA REALIZAR EL CLICK:
	...
	public void borrar(By locator, File rutaCarpeta, String generarEvidencia, int tiempo, String steps)
		...
		if (generarEvidencia.equals("SI")) {
			try {
				...
				ScreenShotError(rutaCarpeta, locator, generarEvidencia, steps, "NO FUE POSIBLE LOCALIZAR EL ELEMENTO: ("
						+ locator + ") PARA BORRAR: " + "\n" + "### ERROR COMPLETO: " + e.toString() + " ###");
				printConsole(e.toString());
				log.error(e.toString());
				generarReportePdf.cerrarPlantilla2();
	...
	public void sendkey(String inputText, By locator, File rutaCarpeta, String generarEvidencia, String steps)
			throws Exception {
		if (generarEvidencia.equals("SI")) {
			try {
				...
				ScreenShotError(rutaCarpeta, locator, generarEvidencia, steps,
	...
	public boolean isPresent(By elementLocation, File folderPath, String steps, String elemento)
		...
		if (isPresent == true) {
			...
			screenshotSencillo(folderPath,
					steps + "La validacion no es exitosa. El elemento " + elemento + " no se encuentra");
	...
	// METODO ENTER (SUBMIT)
	public void submit(By locator, File rutaCarpeta, String generarEvidencia, String steps, int tiempo)
			throws Exception {
		if (generarEvidencia.equals("SI")) {
			try {
				...
				ScreenShotError(rutaCarpeta, locator, generarEvidencia, steps,
						"NO FUE POSIBLE LOCALIZAR EL ELEMENTO: (" + locator + ") PARA REALIZAR EL SUBMIT: " + "\n"
								+ "### ERROR COMPLETO: " + e.toString() + " ###");
				log.error(e.toString());
				printConsole(e.toString());
				generarReportePdf.cerrarPlantilla2();
	...
	// METODO SCROLL
	public void scrollWeb(int y, int numMovimiento, File rutaCarpeta, String generarEvidencia, int tiempo, String steps)
			throws Exception {
		if (generarEvidencia.equals("SI")) {
			try {
	...
	public void scrollElement(By locator, File rutaCarpeta, String generarEvidencia, String steps) throws Exception {
		...
		try {
			...
			ScreenShot(rutaCarpeta, generarEvidencia, steps);
			// ((JavascriptExecutor)
			// driver).executeScript("arguments[0].scrollIntoView(true);",new
			// WebDriverWait(driver,
			// 20).until(ExpectedConditions.visibilityOfElementLocated(locator)));
		} catch (Exception e) {
	...
	public void clickElement(By locator, int x, int y, File folderPath, String steps, String generarEvidencia)
			throws Exception {
	...
	public boolean displayed(By locator, File rutaCarpeta, String generarEvidencia, String steps) throws Exception {
		if (generarEvidencia.equals("SI")) {
			try {
				...
				ScreenShotError(rutaCarpeta, locator, generarEvidencia, steps,
						"LA VALIDACIÓN NO FUE COMPLETADA CORRECTAMENTE, EL ELEMENTO (" + locator
								+ ") NO SE ENCONTRÓ DESPLEGADO EN LA PÝGINA: " + "\n" + "### ERROR COMPLETO: "
								+ e.toString() + " ###");
	...
	public String textoValidacion(By locator, File rutaCarpeta, String steps)
	...
	// METODO QUE ALMACENA EL TEXTO DE UN LOCALIZADOR
	public String textoValidacionSinEvidencia(By locator) throws IOException, DocumentException, InterruptedException {
	...
	// METODO VALIDACIÓN DEVUELVE UNA RESPUESTA BOLEANA DE VERDADERO O FALSO QUE SE
	// PUEDE USAR PARA VALIDAD OBJETOS
		...
		try {
			...
			if (Evidencia.equals("SI")) {
				...
				// En caso contrario imprimira otro mensaje
					...
					screenshotSencillo(rutaCarpeta,
...
// GENERAR PDF -->
	...
	// SALTO DE PAGINA
	public void jumpPage(File folderPath, String steps, String generarEvidencia) throws Exception {
